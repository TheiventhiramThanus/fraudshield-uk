...
export default function TrainingCandidatesPanel() {
  ...
  return <section><div className="mb-3 flex items-end justify-between gap-4"><div><h2 className="font-semibold text-[#0d1b3e]">Training data review</h2><p className="mt-1 text-xs text-slate-500">Only anonymised, user-consented samples appear here. Samples are auto-approved; inspect and exclude unsuitable entries before offline training.</p></div><div className="flex gap-3"><button onClick={exportApproved} disabled={approved.length === 0} className="text-sm text-violet-700 disabled:text-slate-400 hover:underline">Export approved ({approved.length})</button><button onClick={loadCandidates} className="text-sm text-blue-600 hover:underline">Refresh</button></div></div>{error && <p role="alert" className="mb-3 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>}{pending.length > 0 && <div className="mb-4 space-y-3">{pending.map((candidate) => <article key={candidate.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-violet-700">{candidate.kind} · suggested: {candidate.suggestedLabel} · {candidate.riskScore}/100</p><p className="mt-2 break-words font-mono text-sm text-slate-700">{candidate.sanitizedInput}</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => review(candidate.id, "approved", "potentially_suspicious")} className="rounded bg-violet-600 px-3 py-2 text-xs font-medium text-white">Approve: suspicious</button><button onClick={() => review(candidate.id, "approved", "lower_risk")} className="rounded border border-blue-200 px-3 py-2 text-xs font-medium text-blue-700">Approve: lower risk</button><button onClick={() => review(candidate.id, "rejected")} className="rounded border border-red-200 px-3 py-2 text-xs font-medium text-red-700">Reject</button></div></article>)}</div>}{approved.length === 0 ? <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">No approved training samples yet.</div> : <div className="space-y-3">{approved.map((candidate) => <article key={candidate.id} className="rounded-xl border border-emerald-200 bg-emerald-50/40 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">{candidate.kind} · {candidate.autoApproved ? "auto-approved" : "admin-approved"} · {candidate.reviewedLabel} · {candidate.riskScore}/100</p><p className="mt-2 break-words font-mono text-sm text-slate-700">{candidate.sanitizedInput}</p><p className="mt-2 text-xs text-slate-500">Category: {candidate.category}</p><button onClick={() => review(candidate.id, "rejected")} className="mt-4 rounded border border-red-200 bg-white px-3 py-2 text-xs font-medium text-red-700">Exclude from training</button></article>)}</div>}</section>;
}
